...
export async function loadProgress(userId) {
  ...
  const fullAttempts = attempts
    .filter((a) => a.mode === "full" && a.percentage != null && !Number.isNaN(Number(a.percentage)))
    .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));

  let fullTestBestPct = null;
  let fullTestLastPct = null;
  if (fullAttempts.length > 0) {
    fullTestLastPct = Math.round(Number(fullAttempts[0].percentage));
    fullTestBestPct = Math.round(
      Math.max(...fullAttempts.map((a) => Number(a.percentage)))
    );
  }
  ...
  return {
    ...
    fullTestBestPct,
    fullTestLastPct,
    fullTestCount: fullAttempts.length,
