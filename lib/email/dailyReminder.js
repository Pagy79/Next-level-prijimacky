 ...
 * From: info@kompasnaskolu.cz via Resend.
 */

import { DEFAULT_APP_URL, normalizeAppUrl } from "../appUrl.js";

export function buildDailyReminderEmail({ nickname, appUrl }) {
  const name = (nickname || "parťáku").trim() || "parťáku";
  const url = normalizeAppUrl(appUrl) || DEFAULT_APP_URL;
  const subject = "Dnes ještě procvič češtinu a posuň raketu dál! 🚀";

  const text = [
    `Ahoj ${name},`,
    "",
    "Dnes si ještě neprocvičoval češtinu, tak pojď raketovou rychlostí na to.",
    "Krátká mise teď posune raketu dál a udrží tvoji sérii naživu.",
    "",
    `Otevři appku: ${url}`,
    "",
    "Tvůj parťák na češtinu",
    "Trénink češtiny · info@kompasnaskolu.cz",
  ].join("\n");
    ...
    <tr>
      <td align="center">
          ...
          <tr>
            <td style="padding:28px 28px 10px 28px;text-align:center;">
              ...
              <h1 style="margin:12px 0 0;font-size:22px;line-height:1.35;color:#f5f3ff;font-weight:800;">
                Dnes si ještě neprocvičoval češtinu,<br/>tak pojď raketovou rychlostí na to.
              </h1>
            </td>
          </tr>
          <tr>
        ...
        <p style="margin:16px 0 0;font-size:11px;color:#5b4b78;">
          info@kompasnaskolu.cz · Tvůj parťák na češtinu
        </p>
      </td>
    </tr>
  </table>
